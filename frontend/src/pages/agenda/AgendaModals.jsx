...
export function DetalhesAgendamentoModal({ open, onClose, agendamento, onUpdate }) {
  ...
  const canStart       = ['agendado', 'confirmado'].includes(ag.status);
  const canComplete    = ['agendado', 'confirmado', 'em_atendimento'].includes(ag.status);
  const canNoShow      = ['agendado', 'confirmado'].includes(ag.status);
        ...
        {!isCanceled && !showEdit && (
          <div className="pt-3 border-t border-primary space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {canConfirm && (
                <button className="flex items-center gap-1.5 px-4 py-2.5 bg-emerald-50 text-emerald-600 border border-emerald-200 rounded-2xl text-sm font-medium hover:bg-emerald-100 transition-all" onClick={() => doAction(confirmarAgendamento)} disabled={loading}>
                  <FiCheck size={14} /> Confirmar
                </button>
              )}
              {canStart && (
                <button className="flex items-center gap-1.5 px-4 py-2.5 bg-violet-50 text-violet-600 border border-violet-200 rounded-2xl text-sm font-medium hover:bg-violet-100 transition-all" onClick={() => doAction(emAtendimento)} disabled={loading}>
                  <FiPlay size={14} /> Em Atendimento
                </button>
              )}
              {canComplete && (
                <button className="flex items-center gap-1.5 px-4 py-2.5 bg-teal-50 text-teal-600 border border-teal-200 rounded-2xl text-sm font-medium hover:bg-teal-100 transition-all" onClick={() => doAction(concluirAgendamento)} disabled={loading}>
